...
namespace MVCTraining
{
    public class FormManager
    {
        ...
        /*
        public void Operate<TargetForm>( BaseForm source, Func<Command, TargetForm, bool> operation)
            where TargetForm : BaseForm
        {
            foreach (var form in _managed_baseform)
            {
                if (form.Invoker == source && form.GetType() == typeof(TargetForm))
                {
                    operation((TargetForm)form);
                }
            }
        }
        */

        public void Operate<TargetForm>(Command command)
            where TargetForm : BaseForm
        {
        ...
        public void OperatePrevious<TargetForm>(Command command)
            where TargetForm : BaseForm
        {
            if (command == null)
            {
